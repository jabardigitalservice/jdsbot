...
def reply_message(
    telegram_item, msg, is_direct_reply=False, is_markdown=False, custom_data=None
):
...
def process_report(
    telegram_item, input_fields, image_data, peserta=None, save_history=True
):
    ...
    )  # clone fields to avoid changing source values
    field_aliases = {
        "tanggal": "dateTask",
    ...
    if req.status_code >= 300:
        errors.append(
            "Groupware status code : {}\n\nMohon maaf, sedang ada ganguan pada sistem groupware. Silahkan coba lagi setelah beberapa saat".format(
                req.status_code
            )
        )
    ...
    def send_result(result):
        reply_message(
            telegram_item, "Hasil:\n" + "\n".join(results), is_direct_reply=True
        )
