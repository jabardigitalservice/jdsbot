...
def action_lapor(item, peserta=None):
    ...
    for line in lines[1:]:
        ...
        if (
            len(val) > 1 and len(val[1]) > 0
        ):  # only process lines which contain field_name & content
    ...
    # if this message is replying to a photo
        ...
        "reply_to_message" in item["message"]
    ...
    return bot.process_report(
        item, data, image_data, peserta=peserta, save_history=(peserta is None)
    )
