...
class TestUser(unittest.TestCase):
    ...
    def test_insert_select(self):
        ...
        db.get_conn().execute(
            query_insert, id=random_id, username="test_user", password="test_password"
        )
        res = [row for row in db.get_conn().execute(query_select, id=random_id)]
    ...
    def test_auth_custom_alias(self):
        ...
        db.get_conn().execute(
            query_insert, username=self.testuser, password=self.testuser
        )
        user.load_user_data()
    ...
    def test_auth_custom_password(self):
        ...
        db.get_conn().execute(
            query_insert, username=self.testuser, password=test_password
        )
        user.load_user_data()
        with self.assertRaises(Exception):
